...
function PostBody({
  ...
  const count = React.useMemo(() => {
    ...
    const hasVideo = React.useMemo(() => {
        ...
        if (el?.attachments?.length) {
            el?.attachments.map(some => {
                if (isVideo(some)) {
                    isVideoParams = true
                }
            })
        }
        return isVideoParams
    })
    ...
    <View key={key} paddingBottom="15px" borderBottomWidth={1} borderBottomColor="#EBEFFF">
      ...
      <HStack position="relative" height="28px" flexDirection="row" alignItems="center" marginTop="12px" marginLeft="15px">
        ...
        <HStack zIndex={9} position="absolute" left={0} right={0} justifyContent="center" space="5px">
          ...
          {hasVideo ? <HStack right={15} position="absolute" zIndex={9}  justifyContent="center" space="8px" alignItems="center">
              <Image alt="show" size="19px" source={require('../../assets/images/icons/Show.png')} />
              <Text fontSize={16} fontFamily="NunitoBold">{el?.views}</Text>
          </HStack> : null}
      </HStack>
